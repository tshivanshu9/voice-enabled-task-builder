...
export const getTaskByTaskId = async (req, res) => {
  try {
    const { id } = req.params;
    const task = await Task.findOne({ _id: id, status: ACTIVE_INACTIVE_ENUM.ACTIVE });
    if (!task) {
...
export const deleteTaskByTaskId = async (req, res) => {
  try {
    ...
    const deletedTask = await Task.findByIdAndUpdate(id, { status: ACTIVE_INACTIVE_ENUM.INACTIVE }, { new: true });
    if (!deletedTask) {
      return res.status(400).json({ error: "Task not found" });
    }
